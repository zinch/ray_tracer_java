...
public record Ray(Point origin, Vector direction) {
    ...
    public RayIntersections intersect(Sphere s) {
        ...
        if (discriminant < 0) {
            return new RayIntersections(null, null);
        }

        var t1 = (-b - Math.sqrt(discriminant)) / (2 * a);
        var t2 = (-b + Math.sqrt(discriminant)) / (2 * a);

        return new RayIntersections(new RayIntersection(t1, s), new RayIntersection(t2, s));
    }
}
